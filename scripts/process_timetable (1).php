...
// --- Reorder sessions: schedule lab-2hr-with-lab-days first, then other subjects, then PE/NSTP last ---
// This ensures specific 2-hour lab blocks are placed first in their allowed timeframes.
$cs_lab2 = [];
$cs_non_pe = [];
$cs_pe = [];
foreach ($cs as $r) {
    $ct = strtolower(trim((string)($r['course_type_norm'] ?? $r['course_type'] ?? '')));
    $neededLab = (int)($r['total_laboratory_class_days'] ?? 0);
    $class_hours = (float)($r['class_hours'] ?? 0.0);
    // lab subjects with 2.0 class hours
    if ($neededLab > 0 && abs($class_hours - 2.0) < 1e-6) {
        $cs_lab2[] = $r;
    } elseif ($ct === 'pe' || $ct === 'nstp') {
        $cs_pe[] = $r;
    } else {
        $cs_non_pe[] = $r;
    }
}
$cs_ordered = array_merge($cs_lab2, $cs_non_pe, $cs_pe);
...
foreach (["1st", "2nd"] as $term) {
    foreach ($cs_ordered as $row) {
        ...
        for ($start = 0; $start <= $totalSlots - $nSlots; $start++) {
            if (spans_lunch($start, $nSlots)) {
                continue;
            }

            // --- Lab-2hr timeframe enforcement ---
            // If this course has lab days and is exactly 2.0 class hours, restrict start to specific slots.
            $is_lab2 = false;
            $row_class_hours = (float)($row['class_hours'] ?? 0.0);
            $row_neededLab = (int)($row['total_laboratory_class_days'] ?? 0);
            if ($row_neededLab > 0 && abs($row_class_hours - 2.0) < 1e-6) {
                $is_lab2 = true;
            }
            if ($is_lab2) {
                // determine allowed starts for this session_group
                $labmap = lab2_allowed_starts_map();
                $sg_row = get_session_group_row($sgid);
                $label = '';
                if ($sg_row !== null) {
                    $label = strtolower(trim((string)$sg_row['session_time'] ?? ''));
                }
                // collect allowed starts: if label empty, allow all lab2 starts (merge all)
                $allowedStarts = [];
                if ($label !== '' && isset($labmap[$label])) {
                    $allowedStarts = $labmap[$label];
                } else {
                    // merge all values
                    foreach ($labmap as $vals) {
                        foreach ($vals as $v) {
                            if (!in_array($v, $allowedStarts)) $allowedStarts[] = $v;
                        }
                    }
                }

                // compute current candidate start HH:MM
                $slotStartDt = slot_dt($start);
                if ($slotStartDt instanceof DateTime) {
                    $cand_hm = $slotStartDt->format('H:i');
                } else {
                    $cand_hm = date('H:i', strtotime((string)$slotStartDt));
                }

                if (!in_array($cand_hm, $allowedStarts)) {
                    // reject candidate if start not one of allowed lab2 starts
                    continue;
                }
            }
            // --- end lab2 enforcement ---
