...
@section('title', 'Assign Course Academic Programs')
    ...
    <div class="w-full p-4">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-xl font-bold mb-0 text-white">
                Assign Academic Programs to {{ $course->course_title }}
            </h1>
            <div class="flex gap-4">
                <!-- Confirm button: yellow -->
                <button type="submit" form="courseProgramsForm"
        ...
        <form id="courseProgramsForm"
              action="{{ route('courses.course-academic-programs.store', $course) }}"
              method="POST">
            @csrf
                ...
                @forelse($unassignedPrograms as $program)
                    <tr class="border-t border-gray-200 hover:bg-gray-50 transition-colors cursor-pointer"
                        ...
                        <td class="px-6 py-3 text-center">
                            <input type="checkbox" name="academic_program_ids[]" value="{{ $program->id }}">
                        </td>
                    </tr>
                @empty
                    <tr>
